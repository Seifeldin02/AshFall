...
public final class SMPCore extends JavaPlugin implements CommandExecutor,TabCompleter {
    ...
    @Override public void onEnable(){
        ...
        weeklyDragon=new WeeklyDragonService(this);graves=new GraveService(this);bulletin=new BulletinService(this);guides=new GuideService(this);messaging=new MessagingService(this);
        ...
        getServer().getPluginManager().registerEvents(new GameplayListener(this,factions,teleports,shop,auctions,bosses,bounties,relics,progress,spawners,spawnClaims,netWorth,merchants,capsules,graves),this);
    ...
    private boolean consoleAdmin(CommandSender sender,String[] args){if(!(sender instanceof ConsoleCommandSender)&&!(sender instanceof RemoteConsoleCommandSender)){CoreUtil.error(sender,"This command is console-only.");return true;}CoreUtil.msg(sender,"Admin accounts: "+trustedAdmins.accountList()+". AuthMe login is required.");return true;}
    ...
    private void adminSectionHelp(CommandSender s,String section){
        switch(section.toLowerCase(Locale.ROOT)){
            ...
            case"maintenance","debug"->adminCommands(s,"Debug / Maintenance","/ashfall border status","/ashfall border apply","/ashfall border restore","/ashfall setspawn","/ashfall reload","/ashfall debug","/ashfall selftest");
            default->adminHelp(s);
        }
    }
    ...
    private void relicHelp(CommandSender s){adminCommands(s,"Relics","/ashfall relic give <player> <key>","/ashfall relic remove <key>");}
    ...
    @Override public List<String> onTabComplete(CommandSender sender,Command command,String alias,String[] args){
        ...
        if(args.length==1)return switch(name){case"f"->{List<String> options=new ArrayList<>(List.of("create","claim","unclaim","borders","networth","leaderboard","relations","ally","truce","storage","invite","accept","kick","leader","coleader","leave","disband","info","tag","deposit","withdraw","expand","sethome","home","homes","delhome","buyhome","history","locate"));options.addAll(publicOnlineNames(sender,""));options.addAll(db.factions().stream().map(Database.FactionRow::name).toList());yield filter(args[0],options);}case"shop"->filter(args[0],List.of("luxury","buy","sell","sellall"));case"settings"->filter(args[0],List.of("account","confirmations"));case"ah"->filter(args[0],List.of("sell","collect","cancel"));case"enderchest"->filter(args[0],sender instanceof Player viewer&&isAdmin(viewer)?List.of("upgrade","page","inspect"):List.of("upgrade","page"));case"events"->filter(args[0],List.of("track"));case"guide"->filter(args[0],List.of("English","العربية"));case"leaderboards"->filter(args[0],List.of("money","networth","factions","bosses","kills","deaths","mobs","bounties","events","playtime"));case"relics"->filter(args[0],List.of("trace"));case"ashfall"->filter(args[0],List.of("help","balance","economy","boss","elite","event","merchant","bulletin","feedback","faction","spawnclaim","relic","border","setspawn","reload","debug","selftest","vanish","spectate","unspectate","audit","shard","progressrepair","cooldowns","bounty","dragon","replay"));case"nickname"->filter(args[0],List.of("random","off"));default->List.of();};
        ...
        if(name.equals("ashfall")&&args.length==2){
            return switch(args[0].toLowerCase(Locale.ROOT)){
                ...
                case"relic"->filter(args[1],List.of("give","remove"));
