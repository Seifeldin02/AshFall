...
public final class SMPCore extends JavaPlugin implements CommandExecutor,TabCompleter {
    ...
    @Override public void onEnable(){
        ...
        weeklyDragon=new WeeklyDragonService(this);graves=new GraveService(this);bulletin=new BulletinService(this);guides=new GuideService(this);messaging=new MessagingService(this);obsidian=new ObsidianDurabilityService(this);
    ...
    private void adminFactionInfo(CommandSender s,String[] args){
        ...
        CoreUtil.msg(s,target.name()+" — "+f.name()+" ["+f.tag()+"] | members="+String.join(",",db.factionMembers(f.id()))+" | bank="+CoreUtil.money(f.balance())+" | claim="+(claim==null?"none":claim.size()+"x"+claim.size())+" | net-worth="+CoreUtil.money(netWorth.value(f.id())));
    }
